    ...
    if( pEvent->GetType() == fw::InputEvent::GetStaticEventType() )
    {
        ...
        if( pInputEvent->GetDeviceType() == fw::InputEvent::DeviceType::Keyboard )
        {
            if( pInputEvent->GetDeviceState() == fw::InputEvent::DeviceState::Pressed )
            {
                ...
                case 'Q':                   { m_Flags |= Mask::RotLeft; }   break;
                case 'E':                   { m_Flags |= Mask::RotRight; }  break;
                case 'R':                   { m_Flags |= Mask::RotUp; }     break;
                case 'F':                   { m_Flags |= Mask::RotDown; }   break;
                case 'M':                   { m_Flags |= Mask::ZoomIn; }    break;
                case 'N':                   { m_Flags |= Mask::ZoomOut; }   break;
                case 'G':                   { m_Flags |= Mask::Raise; }     break;
                case 'H':                   { m_Flags |= Mask::Lower; }     break;
