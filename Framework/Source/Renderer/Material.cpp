...
uint64_t Material::GetBGFXRenderState() const
{
    ...
    if (m_Debug_PointMode)
    {
        bgfxRenderState |= BGFX_STATE_PT_POINTS;
        bgfxRenderState |= BGFX_STATE_POINT_SIZE(10);
    }
    
    bgfxRenderState |= (uint64)BGFX_STATE_DEPTH_TEST_LEQUAL + (uint64)m_DepthTest;
